...
def Fallen_about_callback(update: Update, context: CallbackContext):
    ...
    if query.data == "fallen_":
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
            text=f"*Menu Bantuan {BOT_NAME}* 🗂️"
             "\n\n────────────────────"
            f"\n*Aktif Sejak»* {uptime}"
            "\n────────────────────"
            f"\n\nPilih tombol di bawah untuk melihat bantuan perintah {BOT_NAME}.",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="Music", callback_data="fallen_support"
                        ),
                        InlineKeyboardButton(
                            text="Manager", callback_data="help_back"
                        ),
                    ],
                    [
                        InlineKeyboardButton(
                            text="Laporkan bug", url=f"https://t.me/IDNrobot"),
                    ],
                    [
        ...
        query.message.edit_text(
            text="*๏ Saya bot streaming telegram dengan beberapa fitur berguna.*"
            f"\n\nPlatform pendukung seperti Youtube, Spotify, Resso, AppleMusic , Soundcloud dll.Jangan ragu untuk menambahkan saya ke kelompokmu.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
